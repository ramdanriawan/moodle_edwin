...
$page = new admin_settingpage('theme_alpha_block4', get_string('settingsblock4', 'theme_alpha'));
        ...
        $page->add($setting);
          ...
          $default = 'Custom FAQ block. You can add up to 20 items. Of course you can copy a code and use it everywhere on the page.';
          $setting = new admin_setting_configtextarea($name, $title, $description, $default);
          $page->add($setting);
          ...
          for ($block4index = 1; $block4index <= $block4count; $block4index++) {
              ...
              $default = '';
              $setting = new alpha_setting_confightmleditor($name, '<span class="rui-admin-no">'. $block4index . '</span>' . $title, $description, $default);
