...
/**
 * A front page layout for the alpha theme.
 *
 * @package   theme_alpha
...
// Hidden sidebar
if (theme_alpha_get_setting('turnoffsidebarfp') == '1' || !isloggedin()) {
    $hiddensidebar = true;
    $navdraweropen = false;
...
$total = 23;
for ($i = 0; $i <= $total; $i++) {
    ${"block" . $i} = theme_alpha_get_setting("block" . $i);
}

$array = array();

for ($i = 0; $i <= $total; $i++) {
    if(theme_alpha_get_setting("displayblock" . $i) == '1') {
        $array[$i] = ${"block" . $i};
    }
    $array[0] = $block0;
}
