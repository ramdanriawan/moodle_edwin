...
if(theme_alpha_get_setting('block20fw') == '1') {
    echo '<div id="heroSlider" class="c-heroimg c-hero-slider wrapper-fw rui-fp-block--20 rui-fp-margin-bottom '.$block20class.'">';
}
else {
    echo '<div id="heroSlider" class="c-heroimg c-hero-slider wrapper-xl rui-fp-block--20 rui-fp-margin-bottom '.$block20class.'">';
}

        echo '<div class="c-hero-container">';

            if($herologo) {
            echo '<div class="c-top-logo"><img src="'.$herologo.'" class="logo" alt="'.$herologotxt.'"/></div>';
            }
            ...
            echo '<div class="hero-slider">';
                ...
                for ($i = 1; $i <= $block20count; $i++) {
                    ...
                    if($imgslidesonly == true) {
                      ...
                      if($sliderclickable == true) { echo '<a href="'.$url.'">'; }
                      ...
                      if($sliderhtml != true) {
                          ...
                          if(!empty($subtitle)) { echo'<h5 class="h5">'.$subtitle.'</h5>'; }
                          if(!empty($title)) { echo'<h3 class="h1">'.$title.'</h3>'; }
                          if(!empty($caption)) { echo'<div class="h3">'.$caption.'</div>'; }
                          echo '</div>';
                      }
                      echo $sliderhtml;    
                      echo '<img src="'.$img.'" alt="" class="c-hero-img w-100 d-inline" />';
        
                      if($sliderclickable == true) { echo '</a>'; }
                      
                      echo '</div>';

                    } else {
                        echo '<div class="c-hero-slider-item c-hero-slider-item--h" style="background-image: url('.$img.');';
                        if($slideheight == true) { echo 'height:'.$slideheight.';'; }
                        echo '">';
...
if(theme_alpha_get_setting('sliderloop') == '1') { echo 'loop: true,';}
if(theme_alpha_get_setting('sliderintervalenabled') == '1') { echo 'autoplayButtonOutput: false, autoplay: true, autoplayTimeout:'.$sliderinterval.',';}
echo '});</script>';
