...
class theme_alpha_mod_lesson_renderer extends mod_lesson_renderer {
    ...
    public function display_eol_page(lesson $lesson, $data) {
        ...
        if ($data->notenoughtimespent !== false) {
            $output .= $this->paragraph(get_string("notenoughtimespent", "lesson", $data->notenoughtimespent), 'center wrapper-fw');
        }

        if ($data->numberofpagesviewed !== false) {
            $output .= $this->paragraph(get_string("numberofpagesviewed", "lesson", $data->numberofpagesviewed), 'center wrapper-fw');
        }
        if ($data->youshouldview !== false) {
            $output .= $this->paragraph(get_string("youshouldview", "lesson", $data->youshouldview), 'center wrapper-fw');
        }
        if ($data->numberofcorrectanswers !== false) {
            $output .= $this->paragraph(get_string("numberofcorrectanswers", "lesson", $data->numberofcorrectanswers), 'center wrapper-fw');
        }

        if ($data->displayscorewithessays !== false) {
            $output .= $this->box(get_string("displayscorewithessays", "lesson", $data->displayscorewithessays), 'center wrapper-fw');
        } else if ($data->displayscorewithoutessays !== false) {
            $output .= $this->box(get_string("displayscorewithoutessays", "lesson", $data->displayscorewithoutessays), 'center wrapper-fw');
        }

        if ($data->yourcurrentgradeisoutof !== false) {
            $output .= $this->paragraph(get_string("yourcurrentgradeisoutof", "lesson", $data->yourcurrentgradeisoutof), 'center wrapper-fw');
        }
        ...
        if ($data->reviewlesson !== false) {
            $output .= '<div class="wrapper-fw">'.html_writer::link($data->reviewlesson, get_string('reviewlesson', 'lesson'),
        ...
        $output .= '<div class="wrapper-fw">'.html_writer::link($url, get_string('returnto', 'lesson', format_string($course->fullname, true)),
                array('class' => 'btn btn-primary')).'</div>';
        ...
        if (has_capability('gradereport/user:view', context_course::instance($course->id))
            ...
            $output .= '<div class="wrapper-fw">'.html_writer::link($url, get_string('viewgrades', 'lesson'),
                array('class' => 'btn btn-primary')).'</div>';
        }
        return $output;
    }
